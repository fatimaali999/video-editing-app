...
POSSIBLE_FFMPEG_PATHS = [
    r"C:\Users\Acer\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.0.1-full_build\bin",
    r"C:\Users\Acer\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.0-full_build\bin",
...
class VideoService:
    ...
    def _generate_subtitles(self, video, options):
        ...
        try:
            ...
            disable_hallucination_filter = options.get('disable_hallucination_filter', False)
            ...
            if disable_hallucination_filter:
                print(f"[SUBTITLE DEBUG] ⚠️ Hallucination filter DISABLED - all segments will be kept")
            ...
            # DISABLED: Audio silence check - letting Whisper handle all audio
            # The hallucination filter will catch any issues
            force_transcription = options.get('force_transcription', True)  # Always force now
            if False and not force_transcription and self._is_audio_silent(audio_path):
                print(f"[SUBTITLE DEBUG] WARNING: Audio appears to be silent or extremely quiet")
                print(f"[SUBTITLE DEBUG] Skipping transcription to prevent hallucinations")
                # Create warning subtitle for silent audio
                segments = [{
                    'start': 0.0,
                    'end': 5.0,
                    'text': '[Audio is silent or too quiet for transcription]',
                    'confidence': 0.0
                }]
            ...
            try:
                ...
                if len(segments) == 0:
                    print(f"[SUBTITLE DEBUG] WARNING: No valid segments after hallucination filtering")
                    print(f"[SUBTITLE DEBUG] Total segments from Whisper: {total_whisper_segments}")
                    print(f"[SUBTITLE DEBUG] All segments were filtered as empty or hallucinated")
                    print(f"[SUBTITLE DEBUG] This usually means:")
                    print(f"[SUBTITLE DEBUG]   1. Audio is silent or very quiet")
                    print(f"[SUBTITLE DEBUG]   2. Whisper is hallucinating repetitive text")
                    print(f"[SUBTITLE DEBUG]   3. Language mismatch (wrong language selected)")
                    print(f"[SUBTITLE DEBUG] TIP: Try disabling hallucination filter or checking audio quality")
                    
                    # Instead of completely failing, create a warning subtitle
                    print(f"[SUBTITLE DEBUG] Creating warning subtitle instead of failing completely...")
                    segments = [{
                        'start': 0.0,
                        'end': 5.0,
                        'text': '[No speech detected - audio may be silent or unclear]',
                        'confidence': 0.0
                    }]
                
                # Generate both SRT and JSON format subtitles
                srt_content, json_data = self._create_subtitles_from_segments(segments, language, style)
                print(f"[SUBTITLE DEBUG] Using REAL Whisper transcription with {len(segments)} segments")
                ...
                print(f"[SUBTITLE DEBUG] Whisper not available: {e}")
                print(f"[SUBTITLE DEBUG] Install with: pip install openai-whisper")
                raise Exception(f"Whisper not installed: {e}")
            ...
            print(f"[SUBTITLE DEBUG] FATAL ERROR generating subtitles: {e}")
            import traceback
            traceback.print_exc()
            # Don't create fake fallback subtitles - just report the error
            if clip:
                clip.close()
            if audio_path and os.path.exists(audio_path):
                os.remove(audio_path)
            # Store error information instead of fake subtitles
            video.outputs["subtitle_error"] = str(e)
            print(f"[SUBTITLE DEBUG] Subtitle generation failed - no fake fallback created")
            raise  # Re-raise to mark video processing as failed

    def _is_audio_silent(self, audio_path):
        """Check if audio is silent - DISABLED for Urdu transcription"""
        # DISABLED: Let Whisper handle all audio, even if quiet
        print(f"[AUDIO CHECK] ⚠️ SILENCE CHECK DISABLED - Processing all audio regardless of volume")
        return False
    ...
    def _get_transcription_options(self, language):
        ...
        if language in ['ur', 'ru-ur']:
            ...
            # Both Urdu and Roman Urdu use same transcription, differ in post-processing
            return {
                **base_options,
    ...
    def _urdu_to_roman(self, urdu_text):
        """Transliterate Urdu script to Roman Urdu (Latin script)"""
        ...
        for char in urdu_text:
            if char in urdu_to_roman_map:
                roman_text += urdu_to_roman_map[char]
            elif char.isspace() or char in '.,!?;:\'"()-':
                roman_text += char
            else:
                # Keep unmapped characters as-is (numbers, English letters, etc.)
                roman_text += char
    ...
    def _post_process_transcription(self, text, language):
        ...
        print(f"[POST_PROCESS] Called with language='{language}', text length={len(text) if text else 0}")
        
        if not text:
            return text
            
        if language == 'ur':
            # Urdu script post-processing
            text = text.strip()
            ...
            # Roman Urdu: Transliterate from Urdu script to Roman
            print(f"[ROMAN URDU DEBUG] Input text (Urdu): {text[:100]}...")
            text = text.strip()
            
            # First clean Urdu script
            urdu_fixes = {
                ' .' : '۔',
                ' ؟' : '؟',
                ' !' : '!',
            ...
            print(f"[ROMAN URDU DEBUG] Before transliteration: {text[:100]}...")
            text = self._urdu_to_roman(text)
            print(f"[ROMAN URDU DEBUG] After transliteration: {text[:100]}...")
    ...
    def _is_hallucinated_text(self, text):
        """Detect if transcribed text is hallucinated/repetitive/nonsense - RELAXED for Urdu"""
        if not text or len(text.strip()) == 0:
            return True
        
        # ULTRA-RELAXED: Only reject the most extreme hallucinations
        text_clean = text.replace(' ', '').replace('\n', '')
        
        # Only check for EXTREME character repetition (like वावावावावावावावावा)
        if len(text_clean) > 30:  # Longer threshold
            unique_chars = len(set(text_clean))
            total_chars = len(text_clean)
            
            # Only reject if less than 8% unique (VERY repetitive)
            if unique_chars / total_chars < 0.08:
                print(f"[HALLUCINATION] Rejected - EXTREME character repetition: {unique_chars} unique chars in {total_chars} total")
                return True
            
            # Only check for VERY long pattern repetitions (15+ times)
            for pattern_len in range(1, 4):
                if len(text_clean) >= pattern_len * 15:
                    for start_pos in range(min(5, len(text_clean) - pattern_len)):
                        pattern = text_clean[start_pos:start_pos+pattern_len]
                        repetitions = 0
                        pos = start_pos
                        while pos + pattern_len <= len(text_clean) and text_clean[pos:pos+pattern_len] == pattern:
                            repetitions += 1
                            pos += pattern_len
                        
                        if repetitions >= 15:  # Much higher threshold
                            print(f"[HALLUCINATION] Rejected - pattern '{pattern}' repeats {repetitions} times")
                            return True
        
        # ONLY check for EXTREME word repetition (80%+ same word)
        words = text.lower().split()
        if len(words) < 3:
            return False  # Allow all short segments
        ...
        if max_freq / len(words) > 0.8:  # VERY high threshold
            print(f"[HALLUCINATION] Rejected - EXTREME word repetition: {max_freq/len(words):.2%}")
            return True
        
        # Remove most other checks - let translations handle variations
        print(f"[HALLUCINATION] ✅ Accepted segment with {len(words)} words")
        return False

    def _translate_segments(self, segments, source_lang, target_lang):
        """Translate subtitle segments from source to target language"""
        print(f"[TRANSLATE] Starting translation: {source_lang} -> {target_lang}, {len(segments)} segments")
        
        try:
            from deep_translator import GoogleTranslator
            print(f"[TRANSLATE] deep-translator imported successfully")
            ...
            print(f"[TRANSLATE] Creating translator: {source} -> {target}")
            ...
            for idx, segment in enumerate(segments):
                try:
                    ...
                    print(f"[TRANSLATE] Segment {idx+1}: '{original_text[:50]}...' -> '{translated_text[:50]}...'")
                except Exception as e:
                    print(f"[TRANSLATE] Error translating segment {idx+1}: {e}, keeping original")
                    translated_segments.append(segment)
            
            print(f"[TRANSLATE] Translation completed: {len(translated_segments)} segments")
            return translated_segments
            
        except ImportError as e:
            print(f"[TRANSLATE] deep-translator not available: {e}, keeping original language")
            return segments
        except Exception as e:
    ...
    def _get_whisper_language_code(self, language):
        ...
        whisper_codes = {
            'en': 'en',
            'ur': 'ur',
            'ru-ur': 'ur',  # Use Urdu model for Roman Urdu (will transliterate later)
            'ar': 'ar',
            'hi': 'hi',
            'es': 'es',
            'fr': 'fr',
