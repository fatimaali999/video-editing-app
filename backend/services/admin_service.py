...
class AdminService:
    ...
    def authenticate_admin(self, email, password):
        ...
        print(f"[DEBUG] Authenticating admin: {email}")
        
        admin_data = self.admins.find_one({'email': email})
        
        if not admin_data:
            print(f"[DEBUG] Admin not found: {email}")
            return None
        
        print(f"[DEBUG] Admin found, checking is_active...")
        if not admin_data.get('is_active', True):
            print(f"[DEBUG] Admin is not active")
            return None
        
        print(f"[DEBUG] Admin is active, checking password...")
        print(f"[DEBUG] Password hash type: {type(admin_data['password_hash'])}")
        ...
        print(f"[DEBUG] Password verified, updating last login...")
